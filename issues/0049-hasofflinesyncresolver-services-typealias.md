...
`HasOfflineSyncResolver` in `Services` typealias exposes resolver to UI layer. `HasPendingCipherChangeDataStore` does NOT exist in `Services.swift` — the data store is passed directly via initializers. Only `HasOfflineSyncResolver` (line 40) is in the typealias. The original DI-2 concern about data store exposure is moot. Consistent with existing project patterns. Enables future U3.

## Code Review References

### From `ReviewSection_DIWiring.md`
