---
id: 101
title: "[T5/RES-6] Manual mock uses 16 fatalError() stubs"
status: closed
created: 2026-02-21
author: claude
labels: [bug]
...
**Resolution:** Compiler enforces conformance; `fatalError()` is runtime-only risk in tests. AP-T5 (Resolved)

**Disposition:** Resolved / Superseded

## Action Plan

*Source: `ActionPlans/Resolved/AP-T5_InlineMockFragility.md`*
...
Accepted design decision — compiler enforces conformance; `fatalError()` is runtime-only risk in tests; adding `AutoMockable` to `CipherAPIService` is broader project decision outside offline sync scope.
