# Action Plan: R1 (PCDS-3) — No Data Format Versioning for `cipherData` JSON
...
1. **Code verification**: `PendingCipherChangeData.swift:43` declares `@NSManaged var cipherData: Data?` which stores JSON-encoded `CipherDetailsResponseModel`. The encode happens in VaultRepository's offline handlers (e.g., `VaultRepository.swift:1003-1004`): `let cipherResponseModel = try CipherDetailsResponseModel(cipher: encryptedCipher); let cipherData = try JSONEncoder().encode(cipherResponseModel)`. The decode happens in `OfflineSyncResolver.swift:168`: `let responseModel = try JSONDecoder().decode(CipherDetailsResponseModel.self, from: cipherData)`.
...
3. **Impact of decode failure**: If `JSONDecoder().decode()` throws in `resolveCreate` (line 168), `resolveUpdate` (line 192), or `resolveSoftDelete` (line 283), the error propagates through `resolve()` and is caught by the per-item catch at `processPendingChanges` line 131-135. The pending record is NOT deleted. On every subsequent sync, this decode failure repeats - creating a permanently stuck item that blocks sync via the early-abort pattern.
