...
- **~~Concern — Error classification may be overly broad~~** **[Resolved — Design Decision]**: The catch-all `catch` block catches any error not matching the preceding patterns. Deep error-path tracing (2026-02-20) confirmed that only 2 error types realistically reach the catch-all: (1) `URLError` from `URLSession` (network unavailability — the intended trigger) and (2) `ResponseValidationError` with HTTP status >= 500 (server down). Four other theoretical error types (`StateServiceError`, `DecodingError`, `NSError`/Core Data, `HTTPResponseError`) could hypothetically reach it but require conditions that are impossible in practice (user logout in sub-millisecond window, server shipping breaking API changes, Core Data corruption, broken HTTP stack). The conservative bias toward data preservation is the correct default for a password manager — feature flags provide a remote kill switch if needed.
...
**Assessment**:
- **Design decision — Delete converted to soft-delete offline**: When a hard delete fails offline, `handleOfflineDelete` actually performs a local deletion and records a `.softDelete` pending change. This is documented in `AP-VR2_DeleteConvertedToSoftDelete.md`. The rationale is that the server API for hard delete doesn't require sending cipher data, but the offline resolver needs the cipher data for conflict resolution. Using soft-delete preserves the data.
...
- **Good**: Preserves the cipher data in the pending change record so the resolver can handle conflicts.
