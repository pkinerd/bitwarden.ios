...
| # | Issue ID | Description | Severity | Complexity | Est. Effort | Source Documents | Notes |
|---|----------|-------------|----------|------------|-------------|------------------|-------|
...
| 2 | **R4** | **Silent sync abort — no logging when sync aborts due to remaining pending changes.** Impossible to distinguish successful sync, skipped sync, or aborted sync from production logs. | Medium | Low | 1-2 lines, 1 file (SyncService.swift) | AP-R4, AP-00, OfflineSyncCodeReview.md, ReviewSection_SyncService.md, Review2/04_SyncService | Trivial fix: add `Logger.application.info()` + `import OSLog`. Zero risk. Should be bundled with any SyncService commit. |
| 3 | **S7** | **VaultRepository-level `handleOfflineDelete` cipher-not-found test gap.** Resolver-level 404 tests exist but the VaultRepository guard clause when `fetchCipher` returns `nil` remains untested. | Medium | Low | ~30-40 lines, 1 test file | AP-S7, AP-00, OfflineSyncCodeReview.md, ReviewSection_VaultRepository.md, Review2/08_TestCoverage | Add `test_deleteCipher_offlineFallback_cipherNotFound_noOp` to VaultRepositoryTests. |
...
| # | Issue ID | Description | What's Done | What Remains | Severity | Complexity | Source Documents |
|---|----------|-------------|-------------|--------------|----------|------------|------------------|
...
| # | Issue ID | Description | Severity | Rationale | Source Documents |
|---|----------|-------------|----------|-----------|------------------|
...
| # | Issue ID | Description | Severity | Complexity | Dependencies | Source Documents |
|---|----------|-------------|----------|------------|--------------|------------------|
...
## Section 5: New Issues from Review2 (Not Yet Triaged)

These issues were identified in the second review pass and have not yet been evaluated for action plans.

### 5a. Code Quality / Cleanup

| # | Issue ID | Description | Severity | Complexity | Source Document |
|---|----------|-------------|----------|------------|-----------------|
| 28 | **R2-A2** | Unused `stateService` dependency in `DefaultOfflineSyncResolver` — injected but never referenced (~4 lines cleanup) | Low | Low | ReviewSection_OfflineSyncResolver.md, Review2/02_OfflineSyncResolver, Review2/00_Main |
| 29 | **R2-DI-5** | DocC parameter block in `ServiceContainer.swift` init has `pendingCipherChangeDataStore` out of alphabetical order | Low | Low | ReviewSection_DIWiring.md |
| 30 | **P2-CS1** | Redundant MARK comment in `CipherView+OfflineSync.swift` after removing `Cipher` extension | Low | Low | OfflineSyncCodeReview_Phase2.md |
| 31 | **R2-MAIN-20** | Error classification do/catch pattern repeated 4 times in VaultRepository (~15 lines each); could extract helper | Low | Low | Review2/00_Main, Review2/03_VaultRepository |
...
| # | Issue ID | Description | Severity | Complexity | Source Document |
|---|----------|-------------|----------|------------|-----------------|
| 35 | **R2-TEST-1** | `GetCipherRequest` 404 validation (`validate(_:)` throwing `OfflineSyncError.cipherNotFound`) has no direct unit test | Medium | Low | Review2/08_TestCoverage |
| 36 | **R2-TEST-2** | Core Data lightweight migration (adding `PendingCipherChangeData` entity) has no automated test | Medium | Medium | Review2/08_TestCoverage |
| 37 | **R2-TEST-3** | `PendingCipherChangeData.deleteByUserIdRequest` addition to batch delete not explicitly tested | Medium | Low | Review2/08_TestCoverage |
| 38 | **R2-TEST-5** | Corrupt `cipherData` in pending change — no test for resolver handling malformed JSON | Medium | Low | Review2/08_TestCoverage |
| 39 | **P2-T2** | `resolveCreate` partial failure (server succeeds, local cleanup fails) — duplicate cipher scenario not tested | Low | Medium | OfflineSyncCodeReview_Phase2.md |
...
| # | Issue ID | Description | Severity | Complexity | Source Document |
|---|----------|-------------|----------|------------|-----------------|
| 43 | **R2-MAIN-7** | No maximum pending change age or count — unbounded accumulation possible during extended offline periods | Low | Low | Review2/00_Main |
| 44 | **R2-RES-2** | Conflict resolution timestamp comparison uses client-side timestamps — device clock skew could select wrong "winner" | Low | Low | Review2/02_OfflineSyncResolver |
| 45 | **R2-VR-1** | Error classification may be overly broad — catch-all block catches ANY unclassified error for offline fallback | Low | Low | Review2/03_VaultRepository |
| 46 | **R2-VR-5** | JSONEncoder().encode in offline helpers could theoretically fail — edit saved but no pending change recorded | Low | Low | Review2/03_VaultRepository |
| 47 | **R2-VR-6** | `getActiveAccountId()` in `handleOfflineDelete` could throw if user logged out between operation start and call | Low | Low | Review2/03_VaultRepository |
| 48 | **R2-PCDS-1** | No Core Data schema versioning step — current entity addition works via lightweight migration but future attribute changes require explicit versioning | Medium | Medium | Review2/01_PendingCipherChangeData |
| 49 | **R2-PCDS-4** | Upsert race condition — fetch-then-insert/update is not atomic; mitigated by uniqueness constraint | Low | Low | Review2/01_PendingCipherChangeData |
| 50 | **R2-PCDS-5** | Core Data corruption risk — if persistent store corrupts, pending changes lost (inherent to Core Data) | Low | High | Review2/01_PendingCipherChangeData |
| 51 | **S8.a** | When feature flag is disabled, existing pending changes remain orphaned in Core Data with no cleanup or notification | Low | Medium | AP-S8 |
| 52 | **P2-T3** | Orphaned pending change cleanup failure — `deletePendingChange` fail after server success causes successful operation to appear as failure to UI | Low | Low | OfflineSyncCodeReview_Phase2.md |
...
| # | Issue ID | Description | Severity | Complexity | Source Document |
|---|----------|-------------|----------|------------|-----------------|
| 56 | **R2-UP-1** | SDK API changes (`.authenticator` to `.vaultAuthenticator`, `emailHashes` removal) need verification against offline sync cipher operations | Medium | Low | Review2/09_UpstreamChanges |
...
| # | Issue ID | Description | Severity | Complexity | Source Document |
|---|----------|-------------|----------|------------|-----------------|
| 60 | **TC-5** | `@MainActor` annotation required on test as workaround for `MockVaultTimeoutService` actor isolation | Low | Medium | ReviewSection_TestChanges.md |
| 61 | **TC-8** | ServiceContainer mock defaults: 131 calls across 107 test files silently receive new offline sync mocks | Low | Low | ReviewSection_TestChanges.md |
| 62 | **R2-RES-4** | Backup naming uses device default timezone — timestamp may not match server time | Low | Low | Review2/02_OfflineSyncResolver |
| 63 | **R2-RES-8** | Batch processing is sequential — many pending changes processed one-by-one (accepted simplicity tradeoff) | Low | Low | Review2/02_OfflineSyncResolver |
| 64 | **R2-VR-4** | Each offline update decrypts previous version to compare passwords — could be costly if offline edits are very frequent | Low | Low | Review2/03_VaultRepository |
| 65 | **TC-4** | `isVaultLocked` value is cached and reused across potentially long-running API calls | Low | Low | ReviewSection_TestChanges.md |
| 66 | **R2-UI-3** | `specificPeopleUnavailable(action:)` alert is upstream change mixed into offline sync files | Low | Low | Review2/06_UILayer |
| 67 | **PCDS-3/4** | `upsertPendingChange` uses fetch-then-update pattern rather than atomic upsert | Low | Medium | ReviewSection_PendingCipherChangeDataStore.md |
| 68 | **RES-vaultLocked** | `.vaultLocked` error case defined but never thrown in current code — dead code | Low | Low | ReviewSection_OfflineSyncResolver.md |
